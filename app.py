    ...
    if request.method == 'POST':
        ...
        if result > 0:
            ...
            if password == hash_password :
                ...
                print("Success login", session['username'])    
    ...
    if request.method == 'GET':
        ...
        if transactionId == '':
            if transactionType == 'in':
                result = cur.execute("SELECT tr.*,  pr.product_name, sp.supplier_name  FROM webapp.transaction AS tr LEFT JOIN supplier AS sp ON tr.`from_to` = sp.`supplier_id` LEFT JOIN product AS pr ON tr.product =  pr.product_id WHERE transaction_type = %s ORDER BY tr.transaction_id ASC", [transactionType])
            else:
                result = cur.execute("SELECT tr.*, pr.product_name, ct.firstname, ct.lastname FROM webapp.transaction AS tr LEFT JOIN customer AS ct ON tr.`from_to` = ct.`customer_id` LEFT JOIN product AS pr ON tr.product =  pr.product_id WHERE transaction_type = %s ORDER BY tr.transaction_id ASC", [transactionType])
            
        else:
            if transactionType == 'in':
                result = cur.execute("SELECT tr.*, pr.product_name, sp.supplier_name FROM webapp.transaction AS tr LEFT JOIN supplier AS sp ON tr.`from_to` = sp.`supplier_id` LEFT JOIN product AS pr ON tr.product =  pr.product_id WHERE transaction_type = %s AND transaction_id = %s ORDER BY tr.transaction_id ASC", [transactionType, transactionId])
            else:
                result = cur.execute("SELECT tr.*, pr.product_name, ct.firstname, ct.lastname FROM webapp.transaction AS tr LEFT JOIN customer AS ct ON tr.`from_to` = ct.`customer_id` LEFT JOIN product AS pr ON tr.product =  pr.product_id WHERE transaction_type = %s AND transaction_id = %s ORDER BY tr.transaction_id ASC", [transactionType, transactionId])
